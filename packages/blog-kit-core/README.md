# @haroonwaves/blog-kit-core

Core utilities for parsing markdown post files with frontmatter. Extract metadata, calculate reading
time, and manage post content in Node and React applications.

## Installation

```bash
npm install @haroonwaves/blog-kit-core
# or
pnpm add @haroonwaves/blog-kit-core
# or
yarn add @haroonwaves/blog-kit-core
```

## Documentation

For complete documentation, API reference, and examples, please visit the
[main documentation](https://blog-kit.haroonwaves.com).

## Quick Links

- [Quick Start](https://blog-kit.haroonwaves.com/docs/quick-start)
- [Core Package Usage](https://blog-kit.haroonwaves.com/docs/core-package)
- [API Reference](https://blog-kit.haroonwaves.com/docs/api-reference)

## Related Packages

- [@haroonwaves/blog-kit-react](https://www.npmjs.com/package/@haroonwaves/blog-kit-react) - React
  components for rendering posts

## License

[LICENSE](/LICENSE)
